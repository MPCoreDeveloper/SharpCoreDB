...
public static class InsertBatchOptimized
{
        ...
        List<DataType> columnTypes)
    {
        ...
        for (int i = 0; i < columns.Count; i++)
        {
            ...
            // Write the value (delegate to serialization logic)
            // For now, just mark as present/null
            ...
            else
            {
                ...
                // This is simplified; actual implementation uses WriteTypedValueToSpan
                switch (type)
                {
                    case DataType.Integer:
                        bytesWritten += SerializeInt32(buffer.Slice(bytesWritten), (int)value);
                        break;
                    case DataType.Long:
                        bytesWritten += SerializeInt64(buffer.Slice(bytesWritten), (long)value);
                        break;
                    case DataType.Real:
                        bytesWritten += SerializeDouble(buffer.Slice(bytesWritten), (double)value);
                        break;
                    case DataType.Decimal:
                        bytesWritten += SerializeDecimal(buffer.Slice(bytesWritten), (decimal)value);
                        break;
                    case DataType.String:
                        bytesWritten += SerializeString(buffer.Slice(bytesWritten), (string)value);
                        break;
                    default:
