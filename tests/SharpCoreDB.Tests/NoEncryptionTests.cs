...
public class NoEncryptionTests : IDisposable
{
    ...
    public void NoEncryption_PerformanceBenefit_IsMeasurable()
    {
        ...
        try
        {
            ...
            swNoEncrypt.Stop();
            (dbNoEncrypt as IDisposable)?.Dispose();
            System.Threading.Thread.Sleep(200);
            ...
            swEncrypted.Stop();
            (dbEncrypted as IDisposable)?.Dispose();
            ...
            Assert.True(swNoEncrypt.ElapsedMilliseconds < 60000,
                $"NoEncrypt mode should complete in reasonable time. Took {swNoEncrypt.ElapsedMilliseconds}ms");
            Assert.True(swEncrypted.ElapsedMilliseconds < 60000,
