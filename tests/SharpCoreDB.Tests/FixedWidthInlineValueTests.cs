...
public sealed class FixedWidthInlineValueTests : IDisposable
{
    ...
    [Fact(Skip = "§4b open item (2026-09-16). Evidence set, all from filtered (sequential) runs: the row IS written inline and reads back correctly in-session; after reopen it returns DBNull; the reopened table is a DIFFERENT instance from the first (reference comparison); a row inserted AFTER reopen reads back correctly; every DirectoryTableFactory construction receives the right config (fixedWidth=True inlineBytes=16); and — with probes in DirectoryTableFactory AND DatabaseExtensions.LoadTables — NEITHER printed during the reopen phase, so neither constructs the table. SingleFileTable now honours an inline capacity (forwarded, with cache invalidation) and the failure persists. The construction site on reopen is therefore still unidentified, and the next step is a debugger breakpoint on the Table/SingleFileTable constructors during an open — the one tool this session lacked. Acceptance criterion before enabling FixedWidthInlineValueBytes.")]
