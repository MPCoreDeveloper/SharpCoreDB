...
- **SQL multi-row `INSERT … VALUES (…), (…), …` now uses the batched insert core — 1.45× on that shape, and it removes a write-through file open per row.** The statement used to lower to one `Table.Insert` call per row, i.e. one standalone `FileOptions.WriteThrough` append each; it now hands the whole statement to `Table.InsertBatch` when nothing needs per-row semantics (no INSERT trigger, no CHECK constraint, no unique secondary index, no active batch update) and the statement carries at least **1,000 rows**. Measured with the new `--multirowinsert` mode (20,000 rows, 1,000 rows/statement, 20 statements, median of 5, same machine, `src/` swapped between the two commits): **2,069.77 → 1,430.30 µs/row (483 → 699 rows/s)**. Two honest caveats. **(a) Failures are now atomic for this shape:** the batch core validates every row before writing any, so a statement that fails part-way inserts nothing, where the per-row loop left the rows it had already written in place — this matches SQLite, where one multi-row INSERT is one unit. **(b) The 1,000-row floor is measured, not decorative:** total time tracks the *statement count* as much as the row count (20 statements of 1,000 rows took 31.8 s; 200 statements of 100 rows did not finish inside 300 s for the same 20,000 rows), so a per-statement cost still dominates small statements and the per-row loop is kept for them — the break-even is not yet measured, so the floor stays until it is. `RETURNING`, `last_insert_rowid()` and `IDatabase.GetLastInsertRowId()` are unchanged. Guarded by `MultiRowInsertBatchingTests` (9 tests).
- **Found while measuring that change, now the top INSERT work item: the overflow arena writes through per variable-length value.** The write-path profiler attributes **100%** of multi-row INSERT time to the validate-and-serialize stamp — **1.47 ms/row** on a fixed-width (`PRIMARY KEY`) table — because `OverflowArena.Write` calls `_storage.AppendBytes`, the same open-per-call `FileOptions.WriteThrough` append measured at **477.97 µs/record**. Every TEXT/JSON/BLOB value above the inline threshold therefore pays a full file open + write-through + close, and `EnableBufferedAppends` does **not** cover the arena (it only routes single-row appends to the table data file). This is a larger lever than the append-buffer work that preceded it, and it needs no format change — it also explains why the non-PK benchmark shape (values inline) inserts at ~7.7 µs/row while the PK/fixed-width shape costs ~1.5 ms/row. Recorded with its next measurement in `docs/performance/INSERT_UPDATE_PERFORMANCE_PLAN.md` §5 item 1b.
