...
- **AES-GCM ciphers were shared across threads — a data-integrity defect off Windows, found by the Linux CI job and fixed by making every cached cipher thread-affine.** `CryptoService` cached one `AesGcm` per key and `AesGcmEncryption` cached one per instance, and both are long-lived objects used from several threads (`CryptoService` is a DI singleton; `DatabaseFile`, `PageEncryption` and the single-file provider each hold one `AesGcmEncryption`). That was wrong on every platform except Windows: dotnet/runtime#53320 states that "AesGcm.Encrypt is thread-safe on Windows but not on other operating systems", with "corruption of the managed buffer, corruption of the underlying native handle, or even nonce reuse (which would destroy GHASH)" among the failure modes. The repository's own two concurrency tests proved it — `CryptoServiceNonceTests.Encrypt_IsThreadSafe_AndNoncesStayUniqueUnderConcurrency` and `AesGcmEncryptionNonceTests.OneInstance_IsSafeUnderConcurrentUse` failed on `ubuntu-latest` with `CryptographicException : Error occurred during a cryptographic operation` out of `AesGcm.EncryptCore` while passing on Windows — and a standalone repro on WSL Ubuntu 24.04 (OpenSSL 3.0.13) pins the mechanism: one shared `AesGcm` across 12 threads fails with `OpenSslCryptographicException: error:1C800066:Provider routines::cipher operation failed`, one instance per call succeeds. Both caches are now keyed by thread (`CryptoService` by (key, thread), `AesGcmEncryption` per thread), which keeps the measured key-import win (0.69 µs of a 1.34 µs `Encrypt` call) with no lock, no contention and no cross-thread native handle. Verified: the CI-filtered suite is **1813 tests / 0 failed / exit 0** on Linux and the core suite **1916 / 0 failed / 16 skipped** on Windows, with five consecutive green Linux runs of the nonce tests.
- **`CryptoService.ResetEncryptionCounter` could pair a pre-reset counter value with the post-reset nonce prefix.** The counter was incremented first and the prefix read afterwards, so a reset landing between the two produced a (new prefix, old-sequence counter) nonce that a later call could legitimately repeat under that same prefix — a GCM nonce repeat, the failure that class documents as defended. `Encrypt` now reads the pair with a seqlock (re-read the prefix after the increment and retry if it changed), so a reset inside a call can no longer mix the two; the hot path pays one cached load.
...
- **The fixed-width record layout now inlines short variable-length values by default (`DatabaseConfig.FixedWidthInlineValueBytes = 16`, was 0).** A fixed-width record stores String/Blob columns as a 5-byte `[null-flag(1)][overflow offset(4)]` slot, so at capacity 0 *every* variable-length value, however short, cost an overflow-arena write. At 16 the slot becomes `[null-flag(1)][offset(4)][inline length(2)][inline payload(16)]` — still a constant size per schema, so the position-stable record and every in-place update and delete fast path are unaffected — and a value that fits is written in the slot. **Measured on the batched multi-row INSERT shape (1,000 rows/statement, median of 5): 62,545 → 70,145–74,634 rows/s (+12 % to +19 %), 15.99 → 13.40–14.26 µs/row, 4,943 → 4,348 B allocated per row, and the overflow arena halved (1,006,670 → 488,890 B).** **The cost is space:** every variable-length column reserves `2 + 16` bytes per record, so that schema's table file grew 2.4× (760,000 → 1,840,000 B), and a value *longer* than the capacity pays the reserve as well as its arena write. Set `FixedWidthInlineValueBytes = 0` to restore the historical layout byte for byte.
  **Upgrade-only, and safe because the layout is self-describing.** The capacity is persisted per table on both storage paths — the multi-file metadata DTO and the single-file `TableMetadataEntry`, where it was carved out of the entry's *reserved* bytes, so the entry size and every field offset are unchanged — and a reopened database decodes with the capacity its records were **written** with rather than with whatever the opening config says. A database created under the old default therefore keeps its capacity-0 layout and needs no rewrite, while tables created from now on get 16. There is no downgrade path: an older build reading a new file would misread its inline slots. This also fixes a defect in its own right — before it, a table created with a non-zero capacity came back from a reopen decoding every inline slot as an arena offset and returning `DBNull` (`FixedWidthInlineValueTests.Reopen_KeepsInlineAndOverflowValues`, previously skipped, now passes).
...
- **A table with an inline capacity lost every inlined value on reopen.** `FixedWidthInlineValueBytes` (plan §4b) is part of the fixed-width record layout, but the layout was computed from the database *config* at decode time — and the reopen path rebuilds each table by JSON-deserializing the `Table` from metadata, so no config survives. A table created with a 16-byte inline capacity therefore came back reading its slots at capacity 0, decoding the inline marker (flag 2) as an overflow-arena offset and returning `DBNull` for every value that had been stored inline; rows inserted *after* the reopen round-tripped correctly, which is what made the defect look inconsistent, and no factory probe fired because the table is built from metadata rather than by a factory. The capacity is now persisted with the layout flag — a settable `Table.FixedWidthInlineValueBytes` (seeded from config at construction, clearing the cached layout on set), the same field on `TableMetadataDto`, and its write in `Database.Core.cs` — so the record layout is **self-describing**: the stored layout is always the one used to decode. Backward compatible by construction, since metadata written before this change has no such field and 0 is the historical layout. `FixedWidthInlineValueTests.Reopen_KeepsInlineAndOverflowValues` is un-skipped and passes (baseline: 17 skipped → 16). This was the acceptance criterion for enabling §4b, whose inline capacity measures **+19.3 % rows/s, −16.2 % µs/row and −12 % allocation** on the batched multi-row INSERT shape (1,000 rows/statement, median of 5), at the cost of a 2.4× larger data file on that schema.
  ⚠️ **The follow-up default change is not part of this fix, and it is blocked on one path.** Making 16 the default was attempted and reverted the same day: it breaks the **single-file (`.scdb`)** variant of the reopen round-trip matrix, because that format still takes the capacity from the opening config rather than storing it — so `SingleFileTable.IsFixedWidthDataBlock`'s record-length test fails on a block whose capacity disagrees, the block falls through to the legacy JSON branch, and `EnsureCacheLoaded` throws `JsonException: '0x14' is an invalid start of a value` on a binary record. That is the same class of defect this entry fixes, one layer out. Persisting the capacity in the SCDB format is the prerequisite, and `Default_InlineCapacity_IsPinned_AndZeroKeepsTheHistoricalLayout` pins the current default so the flip has to be deliberate rather than accidental.
- **The batched append path allocated 64 KiB per call whatever it was writing.** `Storage.AppendBytesMultiple` opened its write stream with a hard-coded `bufferSize: 65536`, and the overflow arena calls that path **once per row** with a single ~46-byte block — so a per-row `INSERT` on a fixed-width table allocated a 64 KiB buffer per row in order to write 46 bytes. Measured over a 20,000-row per-row-statement pass: **76,462 B allocated per row and 244 gen0 collections** before, **15,023 B and 49** after, with `arena-append` falling from 65,856 to 4,416 B/call. The buffer is now sized to the payload and clamped to 4096..65536, so small writes stay buffered and a genuine multi-row batch keeps the coalescing it had (a ~3,000-payload batch still receives 65,536). Durability, file sharing and the record format are untouched. The wall clock on that shape is unchanged and is expected to be: it is bound by the per-value open and flush-to-disk, so this is an **allocation and GC win (5×) rather than a speed win** — the speed story for that posture is `EnableBufferedAppends`, measured at 34×. Found by splitting the append with new `append-open`/`append-write` profiler stages after the per-call allocation had been cross-checked against the single-value path, which showed 4,371 B/call for the same method.
- **The query cache's capacity gate read `ConcurrentDictionary.Count` on every miss, and on a workload whose statements all differ that cost 25 µs per statement.** `Count` is not O(1) — it takes the dictionary's locks and counts every entry — and `QueryCache.GetOrAdd` consulted it on each miss, so a per-row `INSERT` with distinct literals (20,000 distinct statements, one per row) paid a full locked count of a ~1,024-entry dictionary per statement. The gate now reads an `Interlocked` counter, incremented on a successful `TryAdd` and decremented per successful eviction removal, reset in `Clear`; `GetStatistics` still reports the dictionary's real count. **Measured on that shape:** 17,332 → **39,269 rows/s** (2.27×), 57.70 → **25.47 µs/row**. Neutral where it should be — the batched shape (1,000 rows/statement) 58,205 → 56,672 rows/s, the default job's SQL path 94,022 → 92,434 INSERT and 67,811 → 67,524 UPDATE, and the `--pk` fair-PK fixed-width UPDATE 55,578 → 56,885 — because those arms bind by parameter (identical statement text) or use one statement per batch, so the gate never fired. The profiler attributed 25.3 of the 26.2 µs of previously unaccounted per-statement time to the tokenisation region that wraps this lookup, while the `Split` it is named after costs ~0.2 µs. Guarded by the full core suite (1915 tests).
...
- **The SQL DML path no longer warms a plan cache that nothing reads — 59.64 → 51.29 µs per standalone statement, allocation 9.75 → 8.32 B/row.** `Database.ExecuteSQL` called `GetOrAddPlan(sql, …, INSERT/UPDATE/DELETE)` on every statement and **discarded the result**, and nothing anywhere reads a DML plan entry: `CachedPlan` is consumed in exactly two places (`Database.Core.cs:939`, the struct-query path, and `ExecuteQuery`, both SELECT paths) and the only reader, `TryGetCachedPlan`, has **no callers at all**. So for DML the whole chain was cost without effect — and it was self-defeating as a warm-up too, because the cache key is built from the normalized SQL *including literal values* (`GetNormalizedSql` collapses whitespace but not literals), so statements that differ only in their literals miss every time while the cache accumulates one unusable entry per distinct statement. Each statement paid a normalized copy, a cache key, a `Split` of the whole statement, a `CachedQueryPlan`, a cache insert and a `DateTime.UtcNow`. Removed from both `ExecuteSQL` overloads; the statement classification that existed only to feed it went with it. The SELECT path keeps its cache, which is consumed and does hit. Measured with `--multirowinsert` at 1 row/statement (median of 5) through the write-path profiler: `parse` fell from **465.8 ms across two stamps to 71.0 ms across one** — ~21 µs of per-statement warm-up replaced by 3.55 µs of actual parsing. Making DML plans *useful* would be a feature rather than this change: the DML path is handed raw SQL, not a plan.
- **PageBased `UPDATE` now has a profile, and it refutes both earlier explanations: most of the cost is in code with no instrumentation.** `--pk-profile [--engine=…]` gives the `--pk` harness the treatment `--multirowinsert` already had — one untimed pass of the UPDATE arm with the write-path profiler on, report printed, timed arms untouched. Back to back in one session the trap reproduces at **3.4×**: AppendOnly **13.38 µs/update (74,757 ops/s)** against PageBased **45.84 µs (21,815 ops/s)**, with the absolute values contention-affected so that it is the ratio and the shares that mean anything. AppendOnly attributes ~79 % of its time — `row-locate` 51.5 % (a single call: the contiguous in-place path), `commit` 29.7 %, `parse` 18.8 %. PageBased attributes only ~24 % — `arena-write` **60.3 %** of that (6.81 µs/update, allocating **2,829 B per update**), `parse` 17.3 %, `commit` 3.1 % — and `in-place-patch`, `engine-write`, `index-maint` and `encode` have **zero calls**, so roughly **34 µs/update sits in code with no stamp at all**. The measured arena traffic is also the first hard evidence for what §6 of the plan had recorded by reading alone: a PageBased update re-serializes the entire record — **2,829 B/update against the INSERT path's ~1.1 KB per row for the same record**. Instrumentation was then added to the batch-update core (`Table.BatchUpdate.cs`, five PageBased branches) and to `UpdatePageBasedRow` (`Table.CRUD.cs`) — every one of them previously emitted nothing — and **none of it fired for this arm**: the batch is dispatched *per statement* into `Table.UpdateAffectedCount`, whose contiguous, PK-lookup and raw-byte fast paths are all gated away from PageBased, so each of the 10,000 statements takes the generic per-op route (`SelectInternal` materialization plus a full re-serialize). The two regions still unstamped in that route are named in plan §6 with line numbers; stamping them is what splits the last ~75 % between materialization, serialization and the page write. **The first of them now has a number:** stamping the `SelectInternal` full materialization and its per-row re-search in `UpdateAffectedCount` gives `row-locate` **5.15 µs/update with 1,022 B/update allocated** on that arm, where a single 1.1 ms stamp had hidden the whole region — so attribution rises from ~24 % to **~31 %** (`arena-write` 8.4, `row-locate` 5.15, `parse` 3.2, `commit` 0.2), and the materialization turns out to be real but **not** the dominant cost. What remains is the generic per-op route *after* the resolve, which is the second region named here and is still unstamped: the PageBased write itself, which the `fastPatch` branch's `engine-write` stamp does **not** cover, because `fastPatch` requires `StorageMode.Columnar` and so never runs on this engine. **The remaining two suspects for the single-row INSERT cost were then stamped too, and both are refuted:** the per-row uniqueness probe is **0.26 µs/statement** (`row-locate`, 31 B/call) and the per-row index maintenance — PK B-tree insert, hash-index adds, B-tree row indexing — is **2.06 µs/statement** (`index-maint`, 1,032 B/statement), so ~30 µs/statement stays inside the `Dispatch` envelope and outside every stage; what is left uninstrumented there is the parser's own statement-execution glue (which the `Parse` stamp does not cover) and `Table.Insert`'s remaining bookkeeping (the unloaded/stale index scans and the per-row `RecordBatchInsert` call that lets a batch update be cancelled). **And a stamp added for the PageBased write missed, informatively:** the `else` of `TryUpdateInPlace` — append a new version, re-point the PK index — sits *inside* `if (StorageMode == StorageMode.Columnar)` (`:2517`), so it is Columnar-only and PageBased never reaches it, which is exactly why the new `engine-write`/`index-maint` stages did not appear on that arm. The PageBased write is therefore still unfound, but now **bounded**: it lies past the Columnar block, in code this session did not reach — the one remaining unknown in the PageBased UPDATE package, with the two stamps added here standing as real coverage for the Columnar update path. **It has since been located by reading past that block:** `Table.CRUD.cs:2627 else // PageBased` — `SerializeRowExact` on `:2629`, the write itself `engine.Update(Name, position, rowData)` on **`:2643`**, then either `RepointIndexesAfterRelocation` or in-place hash moves. Two stamps there (serialize → `Encode`, the engine call → `engine-write`) are all the PageBased arm still needs, so the search is closed and only the stamping remains. **And the last "concrete" suspect for the single-row ~30 µs died to a read rather than a run:** `RecordBatchInsert` (`Database/Transactions/Database.BatchUpdateTransaction.cs:316`) opens with `if (!_batchUpdateActive || primaryKey == null) return;`, so on a plain insert it is a bool test and a return — it cannot be the missing 30 µs. That leaves the unloaded/stale index scans in `Table.Insert` and the parser's `ExecuteInsert` glue as the only named candidates.
- **The A1 coupling is removed — `Async` means the WAL again and buffering is the explicit `EnableBufferedAppends` opt-in — worth +20 % to +67 % on the mixed workload and free for bulk.** The decision recorded above was taken and implemented: `Storage.BuffersAppends` is now `enableBufferedAppends` alone (the `|| asyncAppends` term and its field are gone), the bulk-oriented callers that had been getting buffering from `Async` ask for it explicitly — **`BulkImport`** and the **Serilog sink** (write-once logs) set `EnableBufferedAppends = true` — and every contract comment that promised the old behaviour is rewritten: `WalDurabilityMode`'s own doc, `Storage.BuffersAppends`, the append path's comment, and the harness arm's. Re-measured on the new build under the same protocol (detached, isolated, medians of 3): `--pk` fixed-width plaintext **INSERT 81,344 → 97,316 (+20 %), READ 110,162 → 120,166 (+9 %), UPDATE 230,722 → 385,668 (+67 %), DELETE 168,804 → 213,727 (+27 %)**, with UPDATE back to **0.8× ahead of SQLite** from 1.24× behind. **Bulk is untouched:** the `--multirowinsert` arm (20,000 rows, 1,000 rows/statement, median of 5) is **17.18 µs/row / 58,205 rows/s write-through** against **17.46 µs/row / 57,277 rows/s buffered** — 1.6 % apart, inside noise — because the multi-row path never used `BuffersAppends`. Tests: the 12 A1 tests are repointed to the opt-in, so they now pass under `FullSync` and thereby prove the opt-in is the cause, and **two new tests pin the new contract** — `Async_Alone_DoesNotBufferTableAppends` (both at-rest settings) and `EnableBufferedAppends_Buffers_AtEitherDurabilityMode` — which is what catches the coupling if it ever returns. Core suite 1897 → **1901 tests, 0 failed**; all nine suites green. The 2026-09-16 §2-protocol re-run of `--pk` flagged a *ratio* regression against SQLite — fixed-width UPDATE was 0.6× **ahead** on 2026-09-15 and 1.24× **behind** on 2026-09-16, while SQLite's own UPDATE rose 6 % — and the suspect was already visible in the code: `Storage.BuffersAppends` is `enableBufferedAppends || asyncAppends` (`Storage.Core.cs:49`, with `asyncAppends` from `:89`), so the term A1 added makes an `Async` arm buffer the version append an UPDATE falls back to. The harness could not test it as documented: `SHARPCOREDB_BUFFERED_APPENDS` only *enables* `EnableBufferedAppends` (`Program.cs:648`) and the tuned `--pk` config hardcodes `Async` (`:634`), so the `||` was already true. The durability mode was therefore made overridable — `SHARPCOREDB_WAL_DURABILITY=fullsync` — which yields a one-variable A/B on the same build. Result (medians of 3, isolated): **UPDATE 230,722 → 356,135 (+54 %), DELETE 168,804 → 216,909 (+28 %), INSERT 81,344 → 99,575 (+22 %), READ flat**, with SQLite's reference within 5 % between the two runs. UPDATE returns to **0.8× ahead** of SQLite, i.e. the 2026-09-15 posture restored. So buffered table appends cost on *every* phase of a workload that interleaves reads with appends — the opposite of A1's standalone 9.4× (1,127.61 → 119.79 µs/row), because A1's shape appends and measures throughput while `--pk` reads (a PK probe per statement) and must make the deferred bytes visible. `WalDurabilityMode` is named for the WAL and A1 made it silently govern table-file appends as well; the choice between scoping it back to the WAL (`Async` = WAL only, table buffering only on the explicit opt-in) and scoping it to bulk-only appends is recorded in plan §8a. ⚠️ An earlier version of this entry claimed the 12 `AsyncDurabilityAppendTests` would stay valid under either option. **That was wrong and is corrected here:** those tests asserted `Async must buffer an append made outside a transaction` and built their fixtures from an `Async` config, so removing the coupling required repointing them to the opt-in — which is also what let them keep passing.
- **The SQL `VALUES` scanner is now slice-based: −69 % on the parse stage, −9.5 % of total per-row allocation, and no measured wall-time change.** `ParseInsertValues` (`SqlParser.DML.cs:654`) appended **one character at a time** to a `StringBuilder` for every literal; it now slices literals straight out of the statement text and keeps that character loop only as the fallback for literals a slice cannot express (an interior toggling quote, as in `'it''s'` → `its`, or an unterminated one) — so the semantics are unchanged by construction. Measured on `--multirowinsert` (20,000 rows, 1,000 rows/statement): **`parse` 66.5 → 20.8 ms (3.33 → 1.04 µs/row, −69 %)**, its allocation **18.4 → 8.5 MB (962 → 443 KB per statement, −54 %)**, total allocation **5,456 → 4,937 B/row (−9.5 %, gen0 17 → 14)** — and the **wall median did not move (17.18 → 17.53 µs/row, inside the noise band)**, which is the honest headline: parse was not on the critical path at the margin. It is now 2.3 % of the pass, so the remaining INSERT budget is table-side (`dispatch` 26.6 %, `table-batch` 21.5 %, encode + arena ~16 %, index ~13 %) — i.e. §4b. Eight new `InsertValuesParsingTests` pin the scanner's four observable rules and were **green against the old implementation before the replacement**; they then caught two of my own mistakes (rule 4 is "an interior literal is emitted unconditionally at its comma, so `('', 2)` stores an empty string while a *trailing* empty literal is suppressed"; and the comma flag must be set where the comma is seen, because a literal whose closing quote ends the tuple also ends at length). All nine suites green (2,465 tests).
- **Comparative CRUD re-benchmarked against SQLite and LiteDB on both engines — `docs/benchmarks/SQLITE_LITEDB_COMPARISON_2026-09-16.md`.** AppendOnly, median of 3, 100K inserts in 10K batches then 10K READ/UPDATE/DELETE by PK (ops/sec): **SharpCoreDB SQL 94,022 / 83,539 / 67,811 / 130,542**, **Direct API 132,610 / 110,708 / 106,303 / 179,292**, **StructRow 141,706 / 113,892**, **SQLite 147,874 / 101,518 / 276,063 / 373,337**, **LiteDB 78,597 / 15,629 / 11,080 / 14,432**. So SharpCoreDB beats LiteDB on every operation in both engines (1.2–1.8× INSERT, 5.3–7.3× READ, 6.1–9.7× UPDATE, 9.0–18.1× DELETE), is ahead of SQLite on READ (1.09× Direct, 1.12× StructRow), level on INSERT (0.90–0.96×, and 1.49× ahead on PageBased StructRow) and behind on single-row UPDATE (0.25–0.39×) and DELETE (0.35–0.48×). LiteDB's weakness is point access rather than bulk write — its INSERT is within 1.9× of everything else while its point operations are 6–26× behind — and BLite remains unmeasurable (its `BsonDocumentBuilder` exposes no public setter API in 4.0.1, caught as a `NotSupportedException` by the harness). ⚠️ **But the UPDATE/DELETE columns of that job are not like-for-like, and this is the important part of the finding:** the default job's SharpCoreDB arms declare no primary key (`Program.cs:738/:858/:977`) so their DML predicates on a non-key column (`WHERE name = …`), while the SQLite arm targets `id INTEGER PRIMARY KEY` — a rowid, in-place page edit. On the fair-PK job, where both engines use `WHERE id = @pk`, the same UPDATE column is **SharpCoreDB 385,668 vs SQLite 325,813 = 1.18× ahead**, READ 120,166 vs 101,695 = 1.18× ahead, INSERT 97,316 vs 196,404 = 0.50× and DELETE 213,727 vs 418,093 = 0.51×. So SharpCoreDB already wins READ and UPDATE on the comparable shape, the two deficits are INSERT and DELETE at ~2×, and the same build produced UPDATE figures spanning 67,811 → 385,668 (5.7×) purely from the DML predicate and route. The report now leads with this caveat.
- **DELETE beats SQLite now — 4.0× faster — because the contiguous bulk-delete fast path was the one route that ignored the deferred-index default.** `TryBulkDeleteContiguousFixedWidth` (`Table.CRUD.cs:4277`) still removed the PK B-tree entries and decoded + removed every loaded hash index unconditionally, while every other delete route went through `DeleteRecordsCore`'s deferred branch — even though `EnableDeferredDeleteIndexes` has been the product default since §7a, and §7 had already measured index maintenance at 58.6–81.9 % of a DELETE batch. A new `--pk-profile-delete` arm (`--pk-profile`'s twin, both in the harness) attributed the batch: **`index-maint` 42.6 %** (7 calls, 3.63 µs/delete) + **`index-decode` 26.9 %** (6 calls, 2.29 µs) = 69.5 %, with `parse` 17.3 % (10,000 calls, 1.47 µs) and `commit` 10.8 %, while `engine-write` and `row-locate` were **one call each** — the single-call stamps proving the fast path *was* engaged, which refutes the routing hypothesis and leaves the eager index work as the cost. The fast path now mirrors `DeleteRecordsCore`: `MarkPrimaryKeyIndexStale` instead of `Index.DeleteBulk`, and the hash decode + removal skipped entirely (deliberately without a stale mark, for §7a's measured reason). Fair PK shape, fixed-width plaintext, median of 3: **DELETE 213,727 → 859,387 ops/s (4.68 → 1.16 µs/delete), 0.51× → 2.2× ahead of SQLite**; legacy layout 127,545 → 359,376 (0.31× → 0.92×); at-rest 207,695 → 413,840 (**1.06× — ahead**). §8's DELETE ≥150K target is therefore met on all three postures. Pinned by `Deferred_ContiguousFixedWidthBatchDelete_DefersIndexMaintenance_AndKeepsTheContract`, which asserts the fast path engaged *and* the deferred contract on it — the pre-existing deferral tests all drive their batches descending, which that path rejects (strictly ascending keys required), so the route had never been covered. All nine suites green (2,455 tests, 0 failed).
